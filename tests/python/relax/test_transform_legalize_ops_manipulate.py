...
def test_reverse_sequence():
    ...
    class Before:
        @R.function
        def main(data: R.Tensor((4, 4), "int32"), seq: R.Tensor((4,), "int32")) -> R.Tensor((4, 4), "int32"):
            gv = R.reverse_sequence(data, seq)
            return gv
    ...
    class Expected:
        @T.prim_func(private=True)
        def reverse_sequence(data: T.Buffer((T.int64(4), T.int64(4)), "int32"), seq: T.Buffer((T.int64(4),), "int32"), T_reverse_sequence: T.Buffer((T.int64(4), T.int64(4)), "int32")):
            T.func_attr({"tir.noalias": True})
            # with T.block("root"):
            for ax0, ax1 in T.grid(T.int64(4), T.int64(4)):
                with T.block("T_reverse_sequence"):
                    v_ax0, v_ax1 = T.axis.remap("SS", [ax0, ax1])
                    T.reads(data[v_ax0, T.int64(0):T.int64(4)], seq[v_ax0])
                    T.writes(T_reverse_sequence[v_ax0, v_ax1])
                    T_reverse_sequence[v_ax0, v_ax1] = data[v_ax0, T.if_then_else(T.Cast("int64", seq[v_ax0]) <= T.int64(1) or T.Cast("int64", seq[v_ax0]) <= v_ax1, v_ax1, T.if_then_else(T.int64(4) < T.Cast("int64", seq[v_ax0]), T.int64(3) - v_ax1, T.Cast("int64", seq[v_ax0]) - v_ax1 - T.int64(1)))]

        @R.function
        def main(data: R.Tensor((4, 4), dtype="int32"), seq: R.Tensor((4,), dtype="int32")) -> R.Tensor((4, 4), dtype="int32"):
            cls = Expected
            gv = R.call_tir(cls.reverse_sequence, (data, seq), out_sinfo=R.Tensor((4, 4), dtype="int32"))
            return gv
