...
def test_cast(input_shapes, from_type, to_type):
    ...
    model_name = testing.gen_model_name(op_type, dim_info, from_type, to_type)

    extend_attrs = {"to": getattr(TensorProto, to_type)}

    input_types = [testing.ONNX_DTYPE_MAPPING[from_type.lower()]]
    output_types = [testing.ONNX_DTYPE_MAPPING[to_type.lower()]]
    ...
    input_data = testing.get_op_input(model_name, input_shapes, {"from_type": from_type, "to_type": to_type})
