...
def get_vector_cast_gt_out(from_vdtype, to_vdtype, saturate, a):
    ...
    # From here, the source data type must be floating and the target data type must be integer.
    # Set `0` to the same data type as `a` to avoid type promotion issues.
    a = np.where(np.isnan(a), np.array(0, dtype=a.dtype), a)
...
def is_invalid_saturate(from_dtype, to_dtype, saturate):
    ...
    if to_dtype.is_floating and saturate is not None:
        return True

    if from_dtype.is_floating and to_dtype.is_integer:
...
@pytest.mark.parametrize("to_dtype", ("int8", "uint8", "int16", "uint16", "float16", "bfloat16"))
...
@pytest.mark.parametrize("saturate", (None, True, False))
@pytest.mark.parametrize("to_dtype", ("int16", "uint16", "int32", "uint32", "float32"))
@pytest.mark.parametrize("from_dtype", ("int8", "uint8", "int16", "uint16", "float16", "bfloat16"))
...
def test_cast_to_narrower_bits(from_dtype, to_dtype, saturate):
    ...
    py_func(a, py_out)
    assert_allclose(py_out[:from_n], gt_out[:from_n])
...
if __name__ == "__main__":
    ...
    test_cast_to_narrower_with_merge("int32", "bfloat16", None)
    test_cast_to_same_bits("uint8", "uint8", None)
    test_cast_to_same_bits("int32", "uint32", True)
    test_cast_to_same_bits("float16", "bfloat16", None)
    ...
    test_cast_to_wider_bits("bfloat16", "float32", None)
    test_cast_to_wider_bits("bfloat16", "int32", True)
    ...
    test_cast_to_narrower_bits("float32", "bfloat16", None)
