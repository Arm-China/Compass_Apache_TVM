...
from tvm.compass.dsl.analysis import extract_prim_func_info
...
def test_sub_func():
    ir_mod = BuildManager().lower(main_func)
    param_infos = extract_prim_func_info(ir_mod).param_infos
...
def test_return_ptr_sub_func_single_out():
    ir_mod = BuildManager().lower(return_ptr_sub_func_single_out)
    param_infos = extract_prim_func_info(ir_mod).param_infos
...
def test_return_ptr_sub_func_multi_out():
    ir_mod = BuildManager().lower(return_ptr_sub_func_multi_out)
    param_infos = extract_prim_func_info(ir_mod).param_infos
    assert param_infos[0].is_input_tensor
    assert param_infos[1].is_output_tensor
    assert param_infos[2].is_output_tensor
