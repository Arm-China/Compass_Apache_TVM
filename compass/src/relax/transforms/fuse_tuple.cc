...
/*!
 * \file compass/src/relax/transforms/fuse_tuple.cc
 */
  ...
  IRModule Fuse() {
    ...
    for (const auto& [gv, func] : mod_->functions) {
      if (gv->name_hint == entry_name_) {
        ...
        if (name_opt.defined() && StrStartsWith(name_opt.value(), target_)) {
...
TVM_FFI_REGISTER_GLOBAL("compass.relax.transform.FuseTuple").set_body_typed(FuseTuple);
