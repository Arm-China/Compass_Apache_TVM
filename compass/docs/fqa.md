...
`CPS_TVM_DISABLE_OP_SPEC_CHECKER` can be used too.

**2. How to set the compute threshold for the pass `PruneCompassSubGraphs`?**

The pass is used to prune low compute-intensive Compass subgraphs by compute threshold.
...
```
In addition, the corresponding override environment variable
`CPS_TVM_COMPUTE_THRESHOLD` can be used too.

**3. How to see the graph snapshoots during the partition stage, e.g., the graph
before the pass `PruneCompassSubGraphs`?**
...
```
In addition, the corresponding override environment variable
`CPS_TVM_DUMP_PARTITIONING_GRAPH` can be used too.
...
Relax IR, so you can use any editor to see which operators are partitioned to
NPU.
...
`CPS_TVM_CONTINUOUS_SIM` just like above.
2. Run as usual.
3. Get opt_continuous_similarity.json at compass_output_*/tvm_compass_subfunc*/optimizer and
