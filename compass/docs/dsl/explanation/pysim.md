...
func_add(a, npu_out)  # Run in Python(Pysim)
```
...
an environment variable `export CPS_TVM_PYSIM_SINGLE_THREAD=TRUE`. After all, multi-threaded
...
```py
if os.getenv("CPS_TVM_PYSIM_SINGLE_THREAD", "true").upper() == "TRUE":
