...
def test_vmax_vmin_imm(dtype):
    ...
    f_vmax(a, py_out)
    testing.assert_allclose(py_out, gt_max)

    aipu_out = np.empty(n, dtype)
    ex(a, aipu_out)
    testing.assert_allclose(aipu_out, gt_max)
    ...
    py_out = np.empty(n, dtype)
    f_vmin(a, py_out)
    testing.assert_allclose(py_out, gt_min)

    aipu_out = np.empty(n, dtype)
    ex(a, aipu_out)
    testing.assert_allclose(aipu_out, gt_min)
...
def test_fail_invalid_builtin(capfd, fail_type):
    with pytest.raises(RuntimeError):
        aipu.tir.BuildManager().build(max_min_fail(fail_type))
