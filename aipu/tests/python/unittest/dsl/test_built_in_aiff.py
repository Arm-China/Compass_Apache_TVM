...
def test_aiff():
    ...
    ctrl, param, act = get_desc(aiff, aipu_out)
    ex(aipu_out, ctrl, param, act)
...
def test_aiff_get_none_type_register():
    with pytest.raises(AssertionError) as exc_info:
        ...
        aipu.tir.Aiff(target="X3_1304", descriptor=ctrl)

    exc_msg = str(exc_info.value)
    expect = "The register with address '0' could not found"
    expect += " in the AIFF function units for target 'X3_1304'."
