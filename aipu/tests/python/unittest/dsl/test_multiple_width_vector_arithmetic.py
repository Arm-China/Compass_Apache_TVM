...
def test_vrpadd(dtype):
    ...
    testing.assert_allclose(py_out[hw_lanes], gt_out[hw_lanes])
    testing.assert_allclose(py_out[3 * hw_lanes], gt_out[3 * hw_lanes])
    testing.assert_allclose(py_out[6 * hw_lanes], gt_out[6 * hw_lanes])

    aipu_out = np.empty(n, dtype=out_dtype)
    ex(a, aipu_out)
    testing.assert_allclose(aipu_out[0], gt_out[0])
    testing.assert_allclose(aipu_out[hw_lanes], gt_out[hw_lanes])
    testing.assert_allclose(aipu_out[3 * hw_lanes], gt_out[3 * hw_lanes])
    testing.assert_allclose(aipu_out[6 * hw_lanes], gt_out[6 * hw_lanes])
...
def test_vqdpa(in0_dtype, in1_dtype, out_dtype):
    vdtype = hw_native_vdtype(in0_dtype)
    hw_lanes = vdtype.lanes
    n = hw_lanes * 10
    ...
    testing.assert_allclose(aipu_out, gt_out, atol=atol)
...
if __name__ == "__main__":
    ...
    test_vqdpa("float16", "float16", "float32")
