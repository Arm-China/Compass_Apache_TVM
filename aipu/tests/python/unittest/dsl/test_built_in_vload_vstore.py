...
def test_vstore_stride(dtype, is_stride_var):
    ...
    func(a, stride, py_out)
    testing.assert_allclose(py_out[indices], gt_out[indices])

    aipu_out = np.empty(n * stride, dtype)
    ex(a, stride, aipu_out)
    testing.assert_allclose(aipu_out[indices], gt_out[indices])
...
def test_vload_vstore_mask():
    ...
    def check_gt(func, gt_out, assert_mask):
        ...
        py_out = np.empty(n, dtype)
        func(a, py_out)
        testing.assert_allclose(py_out[assert_mask], gt_out[assert_mask])

        aipu_out = np.empty(n, dtype)
        ex(a, aipu_out)
        testing.assert_allclose(aipu_out[assert_mask], gt_out[assert_mask])
...
def test_vload_vstore_vector_mask():
    ...
    prim_func(a, b, py_out)
    testing.assert_allclose(py_out[::2], gt_out[::2])

    aipu_out = np.empty(n, dtype=dtype)
    ex(a, b, aipu_out)
    testing.assert_allclose(aipu_out[::2], gt_out[::2])
