...
def gen_printf(vdtype, printf_type):
    ...
    def printf_d_i_u_f_func(a: S.ptr(vdtype, "global"), out: S.ptr(vdtype, "global")):
        ...
        S.printf("Test %%f: %f\n", 0.0000546)
        # AIPU result wrong, got "-42949672954294967295.000000", "42949672954294967295.000000".
        S.printf("Test %%f: %f, %f\n", fp32_min, fp32_max)
...
def test_printf(printf_type, capfd):
    ...
    if capfd is not None:
        aipu_stdout, _ = capfd.readouterr()
        for expect in expects:
            msg = f"\nExpect snippet:\n{expect}\n\nAIPU Standard Output:\n{aipu_stdout}\n"
            assert expect in aipu_stdout, msg
            times = aipu_stdout.count(expect)
            assert times == 4, f'[AIPU] The expect snippet appear "{times}" times:\n'
