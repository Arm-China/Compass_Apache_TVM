...
from tvm.aipu.utils import rand, get_binary_op_result_type
...
def test_ceildiv(dtype, divisor_dtype):
    ...
    aipu_out = np.empty(n, out_dtype)
    ex(a, divisors, aipu_out)
...
def test_ceildiv_imm(dtype):
    ...
    aipu_out0, aipu_out1 = np.empty(1, out0_dtype), np.empty(1, out1_dtype)
    ex(a, aipu_out0, aipu_out1)
...
def test_fail_zero_division():
    ...
    aipu.tir.BuildManager().build(func)
