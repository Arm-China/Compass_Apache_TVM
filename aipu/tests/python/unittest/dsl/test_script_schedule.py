...
def test_split():
    sch = aipu.tir.schedule.Schedule(add)
    block_b = sch.get_block("B")
    (i,) = sch.get_loops(block_b)
    sch.split(i, factors=[4, None])

    split = aipu.tir.schedule.Schedule(add_split)
...
def test_vectorize():
    ...
    vectorize = aipu.tir.schedule.Schedule(add_vectorize)
...
def test_bind():
    ...
    bind = aipu.tir.schedule.Schedule(add_bind)
...
def test_compute_at():
    ...
    compute_at = aipu.tir.schedule.Schedule(add_compute_at)
...
def test_schedule_combine():
    ...
    combine = aipu.tir.schedule.Schedule(add_combine)
