...
def test_pointer():
    ...
    ex = aipu.tir.BuildManager().build(pointer_func)
...
def test_pointer_arry_overflow():
    ...
    with pytest.raises(IndexError):
        _ = aipu.tir.BuildManager().build(pointer_func)
...
def test_nullptr():
    ...
    testing.assert_allclose(py_out0, gt_out)

    aipu_out0 = np.empty(n, dtype=dtype)
    ex(inp0, inp1, aipu_out0, out1)
    testing.assert_allclose(aipu_out0, gt_out)
