...
/*!
 * \file aipu/src/target/source/codegen_aipu_v2.cc
 */
...
CodeGenAipuV2::CodeGenAipuV2(AipuInfo aipu_info) : aipu_info_(aipu_info) {
  restrict_keyword_ = "restrict";
  decl_stream << "#include <aipu/tvm_aipu.h>\n\n";
  return;
}

void CodeGenAipuV2::AddFunction(const GlobalVar& gvar, const PrimFunc& f) {
...
void CodeGenAipuV2::InitFuncState(const PrimFunc& f) {
...
void CodeGenAipuV2::PrintFuncPrefix(std::ostream& os) { os << attrs_; }

void CodeGenAipuV2::BindThreadIndex(const IterVar& iv) {
...
std::string CodeGenAipuV2::Finish() {
...
void CodeGenAipuV2::PrintType(DataType t, std::ostream& os) {  // NOLINT(*)
  int lanes = t.lanes();
  if (t.is_handle()) {
  ...
  LOG(FATAL) << "Cannot convert type " << t << " to AIPU type";
}

std::string CodeGenAipuV2::PrintType(DataType t) {
  std::ostringstream os;
  PrintType(t, os);
  return os.str();
}

void CodeGenAipuV2::PrintType(const Type& type, std::ostream& os) {  // NOLINT(*)
...
void CodeGenAipuV2::PrintStorageScope(const std::string& scope, std::ostream& os) {  // NOLINT(*)
  if (scope == "" || StrStartsWith(scope, "global")) {
    // The scope of the cases that lowered from Relay is "".
    os << "__global ";
...
void CodeGenAipuV2::VisitExpr_(const CastNode* op, std::ostream& os) {
...
std::string CodeGenAipuV2::GetBufferRef(DataType dtype, const BufferNode* buffer, PrimExpr index) {
...
void CodeGenAipuV2::PrintCallExtern(Type ret_type, String global_symbol,
...
void CodeGenAipuV2::VisitExpr_(const CallNode* op, std::ostream& os) {
  if (op->op.same_as(builtin::address_of())) {
    // Overload tvm_address_of to add storage scope (e.g. __global).
    const BufferLoadNode* load = op->args[0].as<BufferLoadNode>();
    ICHECK(op->args.size() == 1 && load);
    ICHECK_EQ(load->indices.size(), 1) << "CodeGenAipuV2 only supports flat memory allocations.";
    os << "((";
    ...
    if (name == "vload") {
      ...
      std::string func_name = op->args[1]->dtype.is_float() ? names[0] : names[1];
      ...
      ICHECK(op->dtype.is_float()) << "There isn't any integer equal-width multiply functions.";
      std::string r_str = this->PrintExpr(op->args[1]);
      std::string mask_str = this->PrintExpr(mask);
...
void CodeGenAipuV2::VisitExpr_(const BroadcastNode* op, std::ostream& os) {  // NOLINT(*)
...
void CodeGenAipuV2::VisitExpr_(const IntImmNode* op, std::ostream& os) {  // NOLINT(*)
  auto dtype = op->dtype;
  ICHECK_LE(dtype.bits(), 32) << "CodeGenAipuV2 IntImmNode only supports types within 32 bits.";
...
void CodeGenAipuV2::VisitExpr_(const FloatImmNode* op, std::ostream& os) {  // NOLINT(*)
  auto dtype = op->dtype;
  ICHECK_LE(dtype.bits(), 32) << "CodeGenAipuV2 FloatImmNode only supports types within 32 bits.";
...
void CodeGenAipuV2::VisitExpr_(const StringImmNode* op, std::ostream& os) {  // NOLINT(*)
  os << "\"" << ::tvm::support::StrEscape(op->value) << "\"";
}

template <typename T>
inline void PrintBinaryExpr(const T* op, const char* opstr, std::ostream& os, CodeGenAipuV2* p) {
  if (op->dtype.lanes() == 1) {
...
void CodeGenAipuV2::VisitExpr_(const AddNode* op, std::ostream& os) {
  PrintBinaryExpr(op, "+", os, this);
}

void CodeGenAipuV2::VisitExpr_(const SubNode* op, std::ostream& os) {
  PrintBinaryExpr(op, "-", os, this);
}

void CodeGenAipuV2::VisitExpr_(const MulNode* op, std::ostream& os) {
  PrintBinaryExpr(op, "*", os, this);
}

void CodeGenAipuV2::VisitExpr_(const DivNode* op, std::ostream& os) {
  PrintBinaryExpr(op, "/", os, this);
}

void CodeGenAipuV2::VisitExpr_(const MinNode* op, std::ostream& os) {
  PrintBinaryExpr(op, "min", os, this);
}

void CodeGenAipuV2::VisitExpr_(const MaxNode* op, std::ostream& os) {
  PrintBinaryExpr(op, "max", os, this);
}

void CodeGenAipuV2::VisitExpr_(const FloorDivNode* op, std::ostream& os) {
  PrintBinaryExpr(op, "/", os, this);
}

void CodeGenAipuV2::VisitExpr_(const AndNode* op, std::ostream& os) {
  os << "(";
...
void CodeGenAipuV2::VisitExpr_(const OrNode* op, std::ostream& os) {
  os << "(";
  this->PrintExpr(op->a, os);
  os << " || ";
  this->PrintExpr(op->b, os);
...
void CodeGenAipuV2::VisitExpr_(const SelectNode* op, std::ostream& os) {
  os << "select(";
...
void CodeGenAipuV2::VisitExpr_(const BufferLoadNode* op, std::ostream& os) {  // NOLINT(*)
...
void CodeGenAipuV2::VisitStmt_(const BufferStoreNode* op) {
...
void CodeGenAipuV2::VisitStmt_(const AllocateNode* op) {
...
void CodeGenAipuV2::VisitStmt_(const LetStmtNode* op) {
...
void CodeGenAipuV2::VisitStmt_(const AllocateConstNode* op) {
