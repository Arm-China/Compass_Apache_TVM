...
def _check_if_model_exists(data_path, model_name, model_type):
    ...
    if os.environ.get("CPS_TVM_GEN_MODEL", None) == "True":
        return False, None
...
def _check_if_cfg_exists(data_path, model_name):
    ...
    if os.environ.get("CPS_TVM_GEN_CFG", None) == "True":
        return False, None
...
def is_model_file_exists(op_type, framework_type, model_name):
    ...
    op_data_dir = os.environ.get(
        "CPS_TVM_OP_DIR", os.path.abspath(f"{__file__}/../../../../../../../../op_files")
    )
    op_data_path = os.path.join(op_data_dir, op_type, framework_type)
    ...
    if framework_type == "tf":
        ...
        raise RuntimeError(f"Framework: {framework_type} is not valid.")

    file_path_list = [os.path.join(op_data_path, f_name) for f_name in f_name_list]
    return all([os.path.isfile(f_path) for f_path in file_path_list])
...
def get_model_cfg_path(model_info: dict, model_type: str, need_cfg_str=True):
    ...
    Args:
        ...
        model_type (str): Original framework, e.g onnx
