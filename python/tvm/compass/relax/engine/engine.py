...
"""Provide a mechanism to process Compass function flexibly."""
import numpy as np
import tvm
from tvm import ir, runtime
from ...runtime import PackedFuncModule
from ..utils import convert_to_tuple


class ForwardEngine:
    """Base class of context managers that determine how to build & run Compass function."""

    current = None

    def __enter__(self):
        self._old_engine = ForwardEngine.current
        ForwardEngine.current = self
        return self

    def __exit__(self, ptype, value, trace):
        ForwardEngine.current = self._old_engine

    def pre_build(self, func):
        """Responsible for processing Compass function before Relax build the whole model."""
        return func

    def build(self, func):
        """Responsible for processing Compass function during Relax build the whole model."""
        raise NotImplementedError


class FunctionData:
    """Base class of structure that store data of Relax function."""
...
class PurePythonForwardEngine(ForwardEngine):
    """Base class of forward engines that build & run Compass function in pure Python code."""
    ...
    def create_func_data(self, func):
        """Responsible for creating object used to store data of the Relax function."""
        raise NotImplementedError

    def build(self, func):
        """Build the Compass function for running it through Python code during Relax build the
        whole model."""
        func_name = func.attrs.global_symbol
        self._name2func_data[func_name] = self.create_func_data(func)

        # The decorator is just used to make the function "wrapper" become a packed function.
        ...
        return PackedFuncModule(func_name, wrapper)
