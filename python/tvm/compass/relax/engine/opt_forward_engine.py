...
"""Execute Compass function through Compass Optimizer."""
import os
import json
import numpy as np
from tvm import nd
...
def _continuous_similarity_statistic(func_data):
    ...
    dump_dir = "opt_dump"
    opt_config = CompassConfig.get().optimizer
    ...
    for n in nodes:
        for out_i, out in enumerate(n.outputs):
            ...
            data_quant_name = f"{cfg.runtime_work_dir}/quant/{n.attrs['layer_id']}_opt_{out_i}_"
            data_quant_name += f"{out.name.replace('/', '_')}.npy"
            np.save(data_quant_name, data_quant)
            data_float = f"{func_data.name}_{n.type.split('.')[-1]}__{n.name.replace('/', '_')}__"
            data_float += f"o{out_i}_{n.outputs[out_i].name.replace('/', '_')}.npy"
...
class OptForwardEngine(PurePythonForwardEngine):
    """Build Compass function for running through Compass Optimizer."""
    ...
    def run(self, func_data, args):
        ...
        continuous_sim = CompassConfig.get().common["continuous_similarity"] == "true"
        if self._is_quant:
    ...
    def pre_build(self, func):
        """Build the Compass function to run on Compass Optimizer before Relax build the whole
        model."""
        ...
        # Get the quantized Compass IR through Compass Optimizer if needed.
        if self._is_quant:
    ...
    def create_func_data(self, func):
        ...
        # Write the Compass IR to disk for Compass Optimizer forward interface.
        cfg = CompassFunctionConfig(func_data.name)
        ...
        # Create the Compass Optimizer forward instance.
