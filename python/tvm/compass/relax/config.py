...
"""Configuration processing of Compass."""
import os
import uuid
import shutil
...
# "CompassConfigObj::_type_key".
@tvm.register_object("compass.relax.CompassConfig")
class CompassConfig(tvm.Object):
    """The singleton Compass configuration.
    ...
    def init_singleton(common, parser, optimizer, gbuilder, runtime):
        _ffi_api.CompassConfig_InitSingleton(common, parser, optimizer, gbuilder, runtime)

    @staticmethod
    def get():
        """The static function that is used to get the global Compass configuration singleton.

        Returns
        -------
        result : CompassConfig
            The global Compass configuration singleton.
        """
        return _ffi_api.CompassConfig_Global()

    @property
    def deploy_dir(self):
        framework = self.parser.get("model_type", "compiled")
        model_name = self.parser.get("model_name", "model")
        return os.path.join(self.common["output_dir"], f"{framework}_{model_name}")
...
class CompassFunctionConfig:
    """Collection of various information used to process an Compass function."""

    _IR_NAME = "nn_model"
    _QUANT_IR_NAME = "quant_nn_model"

    def __init__(self, func_name):
        self.target = CompassConfig.get().gbuilder["target"]
        self.func_name = func_name
        refined_name = func_name.replace("@", "_")
        self.output_dir = f'{CompassConfig.get().common["output_dir"]}/{refined_name}'
        ...
        self.gsim_options = ""
        if CompassConfig.get().common["simplify"].upper() in ["FALSE", "0"]:
            self.use_gsim_float = False
        else:
            self.gsim_options = CompassConfig.get().common["simplify"].strip().split(" ")
    ...
    def gen_optimizer_config_file(self, extra_cfg=None):
        ...
        optimizers = CompassConfig.get().optimizers
        idx_str = self.func_name.split("_")[-1]
        opt_cfg = dict(optimizers[idx_str if idx_str in optimizers else "0"])
        # Generate the configuration file of Compass Optimizer and write to disk.
    ...
    def gbuilder_cmd(self):
        ...
        for k, v in CompassConfig.get().gbuilder.items():
            cmd.append(f"--{k}")
            if v != "":
                cmd.append(v)
        return cmd
...
def _get_simulator_path(target_major_mark):
    # Prefer the Compass simulator in environment variable "PATH", because the simulator record in
    # the AIPUBuilder user configuration file maybe an old and invalid one.
    ...
    builder_cfg_path = f'{os.path.expanduser("~")}/.aipubuilder/config'
    if os.path.isfile(builder_cfg_path):
        with open(builder_cfg_path) as f:
            builder_cfg = json.loads(f.read())
        if "simulator" in builder_cfg:
            sim_cfg = builder_cfg["simulator"]
    ...
    raise RuntimeError("Can't find Compass simulator.")
...
def _update_and_check_parser(parser, cfg_dir):
    ...
    if "input_model" in parser:
        value = parser["input_model"]
        value = value.replace("__CURRENT_CFG_DIR__", cfg_dir) if cfg_dir else value
        parser["input_model"] = os.path.abspath(os.path.expanduser(os.path.expandvars(value)))
    return parser
...
def config_compass(config):
    """Initialize the Compass configuration singleton using the information from AIPUBuilder
    configuration file.
    """
    cfg_str = config
    cfg_path = None
    ...
    value_from_env = os.environ.get("CPS_TVM_FORWARD_ENGINE", None)
    if value_from_env:
        value = value_from_env
    common["forward_engine"] = value.lower() if value else "driver"

    value = common.get("continuous_similarity", None)
    value_from_env = os.environ.get("CPS_TVM_CONTINUOUS_SIM", None)
    if value_from_env:
        value = value_from_env
    common["continuous_similarity"] = value.lower() if value else "false"
    continuous_sim = common["continuous_similarity"] == "true"
    if continuous_sim:
        msg = "The forward_engine has been changed to opt_int mode, because continuous sim requires"
        WARN(msg + " every layer results inferred by opt_int.")
        common["forward_engine"] = "opt_int"

    value = common.get("calibrate_collector", None)
    value_from_env = os.environ.get("CPS_TVM_CALIBRATE_COLLECTOR", None)
    if value_from_env:
        value = value_from_env
    common["calibrate_collector"] = value.lower() if value else "relax_vm"

    value = common.get("disabled_pass", None)
    value_from_env = os.environ.get("CPS_TVM_DISABLED_PASS", None)
    if value_from_env:
        value = value_from_env
    common["disabled_pass"] = value or "ForwardFoldScaleAxis"

    value = common.get("log_level", None)
    value_from_env = os.environ.get("CPS_TVM_LOG_LEVEL", None)
    if value_from_env:
        value = value_from_env
    common["log_level"] = value.upper() if value else "INFO"

    value = common.get("log_file", None)
    value_from_env = os.environ.get("CPS_TVM_LOG_FILE", None)
    if value_from_env:
        value = value_from_env
    common["log_file"] = value if value else ""

    value = common.get("disable_op_spec_checker", None)
    value_from_env = os.environ.get("CPS_TVM_DISABLE_OP_SPEC_CHECKER", None)
    if value_from_env:
        value = value_from_env
    common["disable_op_spec_checker"] = value.lower() if value else "false"
    if common["disable_op_spec_checker"] == "true":
        msg = "The Compass OP Spec checkers are disabled, please notice the generated Compass IR "
        WARN(msg + "maybe invalid.")

    value = common.get("compat_quantized_model", None)
    if not value:
        value = parser.get("compat_quantized_model", None)
    value_from_env = os.environ.get("CPS_TVM_COMPAT_QUANTIZED_MODEL", None)
    if value_from_env:
        value = value_from_env
    ...
    for _, optimizer in optimizers.items():
        ...
        value_from_env = os.environ.get("CPS_TVM_OPTIMIZER_SAVE_STATISTIC_INFO", None)
        if value_from_env:
            optimizer["save_statistic_info"] = value_from_env
        optimizer.setdefault("cast_dtypes_for_lib", "true")
        if continuous_sim:
            msg = "The optimizer dump has been changed to true, because continuous sim requires opt"
            WARN(msg + " dump float results.")
            optimizer["dump"] = "true"

    # 6. Update and check "gbuilder" section.
    value = gbuilder.get("target", None)
    value_from_env = os.environ.get("CPS_TVM_GBUILDER_TARGET", None)
    if value_from_env:
        value = value_from_env
    gbuilder["target"] = value.upper() if value else "X1_1204"

    value = gbuilder.get("profile", None)
    value_from_env = os.environ.get("CPS_TVM_GBUILDER_PROFILE", None)
    if value_from_env:
        value = value_from_env
    gbuilder["profile"] = value.lower() if value else "false"

    value = gbuilder.get("tcm_size", None)
    value_from_env = os.environ.get("CPS_TVM_GBUILDER_TCM_SIZE", None)
    if value_from_env:
        value = value_from_env
    if value:
    ...
    value_from_env = os.environ.get("CPS_TVM_RUNTIME_DUMP", None)
    if value_from_env:
        value = value_from_env
    runtime["dump"] = value.lower() if value else "false"

    if runtime["dump"] == "true":
        tvm.register_func("compass.runtime.dump_tensors", _dump_tensors, True)
    ...
    # 8. Initialize the singleton Compass configuration.
    CompassConfig.init_singleton(common, parser, optimizers, gbuilder, runtime)
