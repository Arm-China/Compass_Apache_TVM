...
def _deserialize_register_config(cps_info, desc, begin_idx, end_idx):
    ret = _new_register_config(cps_info)
    cur_idx = begin_idx
    ...
    while cur_idx < end_idx:
        ...
        for i in range(count):
            ...
            msg += f" in the AIFF function units for target '{cps_info.name}'."
...
def _create_register_configs(cps_info, desc):
    if desc is None:
        return [_new_register_config(cps_info)]
    ...
    while eof_or_noe != DESC_CHAIN_EOF:
        ...
        reg_cfg = _deserialize_register_config(cps_info, desc, i + 8, i + cur_length_in_u32)
        ret.append(reg_cfg)
...
class Aiff:
    ...
    def __init__(self, target="X2_1204", descriptor=None):
        self._cps_info = CompassInfo.get(target)
        msg = f'The Compass DSL AIFF does not support the target "{self._cps_info.name}".'
        assert self._cps_info.version != "X1", msg
        self.reg_cfgs = _create_register_configs(self._cps_info, descriptor)
    ...
    def add_new_register_config(self, idx=None, copy_idx=None):
        if copy_idx is not None:
            ...
            new_reg_cfg = _new_register_config(self._cps_info)
    ...
    def gen_descriptor(self):
        ...
        act_chain = ActDescChain(act_descs, self._cps_info)
