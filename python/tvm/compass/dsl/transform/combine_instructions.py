...
def _try_expand_mask(mask, lanes, part):
    lanes_diff = lanes - mask.dtype.lanes
...
class _Mutator(tir.StmtExprMutator):
    def _try_pattern_vnsr(self, vcast):
        ...
        from_dtype, to_dtype = vcast_inputs[0].dtype, vcast.dtype
    ...
    def _try_pattern_rounded_vcast(self, vcast):
        # convert_int8(S.vrint(va)) -> vcvt(va)
        # convert_int8(vcvtu{e_o_h_l}_tfp32(S.vrint(va)) -> vcvtu{e_o_h_l}_tw(va)
        rdtype = vcast.dtype
        if vcast.args[0].value != "convert_int8" or rdtype != "int32x8":
            return None
        _, arg0 = vcast.args
        if is_builtin(arg0, "__vrint"):
            origin_x, vrint_mask = arg0.args[2:]
            if not is_all_true_pred(vrint_mask):
                return None
            if origin_x.dtype == "float32x8":
                return tir.call_extern(rdtype, "__vcvt", origin_x)

        if is_builtin(
            arg0, ("__vcvtue_tfp32", "__vcvtuo_tfp32", "__vcvtul_tfp32", "__vcvtuh_tfp32")
        ):
            e_o_h_l = arg0.args[0].value.split("_")[-2][-1]
            _, vrint = arg0.args
            if is_builtin(vrint, "__vrint"):
                origin_x, vrint_mask = vrint.args[2:]
                if not is_all_true_pred(vrint_mask):
                    return None
                if origin_x.dtype in ("float16x16", "bfloat16x16"):
                    return tir.call_extern(rdtype, f"__vcvtu{e_o_h_l}_tw", origin_x)
        return None
    ...
    def visit_call(self, call):
        ...
        combine_ret = self._try_pattern_rounded_vcast(ret)
        if combine_ret is not None:
            return self.visit(combine_ret)
