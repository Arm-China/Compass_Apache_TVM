...
_default_funcs += ("__vbrevs", "__vpcnt", "__vmax", "__vmin", "__vclass", "verf", "vexp2")
_default_funcs += ("vmul_hi",)
...
class _Mutator(tir.StmtExprMutator):
    ...
    def _mutate_const_pred(self, call):
        ...
        # Zhouyi NPU's vector register is 256-bit, and can't be split into small parts for use, so
        # if we want to load a int8x8 data, the predicate must be set to 0x0000_00FF. For those data
        # whose total bits < 256, need to complement "false" to the high position.
    ...
    def _get_new_lanes(self, old_args, new_args):
        ...
        for old_arg, new_arg in zip(old_args, new_args):
            old_lanes = old_arg.dtype.lanes
            new_lanes = new_arg.dtype.lanes
    ...
    def _mutate_vcast(self, call):
        ...
        if len(inputs) > 1:
            ...
            assert sum(x.dtype.lanes for x in inputs) <= to_hw_lanes, _ERR_IN_SPLIT_MSG
            return tir.Call(to_dtype.with_lanes(to_hw_lanes), ret.op, new_args, ret.span)

        from_bits, to_bits = inputs[0].dtype.bits, to_dtype.bits
    ...
    def _mutate_vall(self, call):
        ...
        old_lanes = call.args[1].dtype.lanes
        new_lanes = ret.args[1].dtype.lanes
        if old_lanes == new_lanes:
            return ret
...
def _pad_params(func, cps_info, mask_asso_dtype):
...
class AlignVectorWidthByPad:
    ...
    def transform_function(self, func, mask_asso_dtype):
        ...
        new_params, var_substitute_map = _pad_params(func, self._cps_info, mask_asso_dtype)
