...
import numpy as np
from tvm import nd, runtime, target as tgt, get_global_func
from ..logger import WARN
from ..utils import sync_compass_output_dir, check_call_compass_tool, control_option
from ..utils import get_rpc_session
from ..runtime import CompassBasicConfig, CompassModule
...
def _gen_aipurun(dir_path, num_inputs, target):
    ...
    file_str += f" -o aipurun_output.bin --target {target}\n"
...
class Executor:
    ...
    def __init__(self, prim_func_info, output_dir, cps_info, has_perf_tick, only_sync_diff=False):
    ...
    def rpc_sess(self):
        ...
        .. code-block:: python
            ...
            from tvm.compass.dsl import get_rpc_session

            ex.rpc_sess = get_rpc_session()
            ex(a, npu_out, 100)  # Run on remote device through RPC.
        """
    ...
    def _check_param_arg_type(self, args):
        ...
        for i, (param_info, arg) in enumerate(zip(self._cur_param_infos, args)):
            if param_info.is_tensor:
                ...
                param_elem_dtype = param_info.dtype.element_of
    ...
    def _update_if_lack_input_tensor(self, args):
        # Do nothing if have input tensor.
        for param_info in self._cur_param_infos:
            if param_info.is_input_tensor:
                return args

        dtype, arg = [(x.dtype, y) for x, y in zip(self._cur_param_infos, args) if x.is_tensor][0]
        self._cur_param_infos.append(ParamInfo("fake_input", dtype, "input_tensor", tensor_idx=0))
        args.append(arg)
        return args

    def _collect_arg_offset(self, args):
        for i, arg in enumerate(args):
            if (not isinstance(arg, np.ndarray)) or arg.flags["C_CONTIGUOUS"]:
                self._arg_offset.append(0)
            else:
                offset = (
                    arg.__array_interface__["data"][0] - arg.base.__array_interface__["data"][0]
                )
                args[i] = arg.base
                self._arg_offset.append(offset)
    ...
    def _build(self, args):
        ...
        gen_gb_plugin(
            self._cur_param_infos, args, op_type, code_name, plugin_path, self._arg_offset
        )

        # 3. Get the Compass executable(i.e., aipu.bin) through "aipugb".
        ...
        cmd += ("--target", self._cps_info.name)
        is_profile = control_option.is_profile
        if is_profile:
            cmd += ("-p", "--enable_dma_perf" if self._has_perf_tick else "--fast_perf")

        check_call_compass_tool(cmd, self._gbuilder_dir)
        ...
        CompassBasicConfig.get().common["output_dir"] = self._output_dir
        cps_bin_path = f"{self._gbuilder_dir}/aipu.bin"
        cps_mod = CompassModule(cps_bin_path, "", is_profile, self._cps_info.name, "")
        ...
        return tgt._ffi_api.AttachCompassModuleToLLVM(cps_mod, llvm_target_str)
    ...
    def _get_packed_funcs(self, args):
        ...
        rt_mod = local_rt_mod
        compass_init = get_global_func("compass.runtime.init")
        if self.rpc_sess:
            export_dir = f"{self._output_dir}/{self._func_name}_{id(local_rt_mod)}"
            os.makedirs(f"{export_dir}/gbuilder", exist_ok=True)
            shutil.copy(f"{self._output_dir}/gbuilder/aipu.bin", f"{export_dir}/gbuilder/aipu.bin")
            device_compiler_path = os.getenv("CPS_TVM_DEVICE_COMPILER")
            local_rt_mod.export_library(f"{export_dir}/nn_model.so", cc=device_compiler_path)

            root_dir, base_dir = os.path.split(export_dir)
            archive_file = shutil.make_archive(f"{export_dir}.cps", "tar", root_dir, base_dir)

            self.rpc_sess.upload(archive_file)
            rt_mod = self.rpc_sess.load_module(os.path.basename(archive_file))
            compass_init = self.rpc_sess.get_function("compass.runtime.init")
    ...
    def _run(self, args):
        ...
        if control_option.is_debug or control_option.is_emu:
            ...
            _gen_aipurun(self._gbuilder_dir, len(in_nd_arrs), self._cps_info.name)
        ...
        if control_option.is_debug:
            ...
            _gen_odb_run(self._gbuilder_dir, self._cps_info.name)
        ...
        if control_option.is_debug or control_option.is_emu:
            for i, nd_arr in enumerate(out_nd_arrs):
                nd_arr.numpy().tofile(f'{self._gbuilder_dir}/npu_output.bin{"" if i == 0 else i}')
    ...
    def _post_profile(self):
        ...
        cmd = ("aipu_profiler", f"{self._gbuilder_dir}/aipu.bin")
        cmd += (f"{runtime_dir}/profile_data.bin", "-o", output_path)
        check_call_compass_tool(cmd, runtime_dir)
    ...
    def run(self, *args):
        ...
        .. code-block:: python
            ...
            a = rand(100, "int8")
            npu_out = np.empty(100, dtype="int8")
            ex.run(a, npu_out, 100)  # Can also be "ex(a, npu_out, 100)".
        """
    ...
    def benchmark(
        ...
        *args,
        ...
        reported. On the Zhouyi NPU specifically, synchonization and data transfer operations are
        not counted towards the runtime. This allows for fair comparison of runtimes across
        different functions and models.
        ...
        .. code-block:: python
            ...
            print(ex.benchmark(a, npu_out, n))
