...
from ..aiff import DescChainArray
from ..run_sim import run_sim
from ..utils import get_binary_op_result_type, resolve_dtype_alias, abspath
    ...
    def _div(self, lhs, rhs):
        ...
        if DataType(lhs.dtype).is_integer:  # Indicate it's a integer division.
            if ret.ndim != 0:  # Indicate it's a integer vector division.
                # Align with Zhouyi NPU, integer vector division will do saturation, because integer
...
def _gen_shift_method(np_func):
    def _method(self, other):
        ...
        # Align with Zhouyi NPU, for the scalar situation, the shift value need to be modulo 32.
        func = (
...
class PyDescPointer:
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(ret, np.ndarray):
            return PyPointer(DataType(ret.dtype), "global", ret.reshape(-1).view("uint8"))
...
class PySimInfo:
    ...
    def __init__(self, cps_info, output_dir, is_multi_thread=True):
        self.cps_info = cps_info
        self.local_size = 4
...
class PyPrimFunc:
    ...
    def _convert_entry_func_arg(self, idx, arg):
        ...
        if isinstance(param, tir.Var):
            ...
            assert param.dtype.is_scalar, msg
            return PyVar(arg, param.dtype)
    ...
    def _dump_output_if_needed(self, args):
        if control_option.is_debug or control_option.is_emu:
            for param, arg in zip(self.param_infos, args):
                if param.is_output_tensor:
                    suffix = "" if param.tensor_idx == 0 else param.tensor_idx
                    arg.np_arr.tofile(f"{self.output_dir}/gbuilder/pysim_output.bin{suffix}")
    ...
    def __call__(self, *args, **kwargs):
        ...
        if os.getenv("CPS_TVM_PYSIM_SINGLE_THREAD", "FALSE").upper() == "TRUE":
            WARN("PySim is running in single thread, some data race issues may can't be caught.")
            with PySimInfo(self.cps_info, self.output_dir, is_multi_thread=False) as py_sim_info:
            ...
            self._dump_output_if_needed(args)
            return None

        with PySimInfo(self.cps_info, self.output_dir, is_multi_thread=True) as py_sim_info:
...
def pysim_run_sim(code_snippet, inputs=None, outputs=None, consts=None, descs=None):
    """Simple run the specified code snippet through Compass simulator during PySim running."""
    ...
    run_sim(pysim_info.cps_info, output_dir, code_snippet, inputs, outputs, consts, descs)
