...
def assert_neither_flexible_nor_multiple_width_vector(dtype):
    ...
    assert dtype.is_scalar or is_hw_native_vdtype_or_mask_type(dtype), msg
...
def assert_not_bfloat16_scalar(dtype):
    msg = "There isn't any compute instruction for bfloat16 scalar, please convert it to "
    msg += "float32 first."
    assert not dtype.is_bfloat16_scalar, msg


# For PySim and IR Builder, a scalar literal is represented by Python built-in type.
def _is_floating_scalar_const(x):
    return isinstance(x, float) or (isinstance(x, str) and x.lower() in ("inf", "-inf", "nan"))


def is_scalar_const(x):
    return isinstance(x, int) or _is_floating_scalar_const(x)


# For IR Builder, a variable (i.e., primitive expression) is represented by class "tir.PrimExpr".
def is_scalar_prim_expr(x):
    return isinstance(x, tir.PrimExpr) and x.dtype.is_scalar


def is_integer_scalar(x):
    return isinstance(x, int) or (isinstance(x, tir.PrimExpr) and x.dtype.is_integer_scalar)


def is_float_scalar(x):
    return _is_floating_scalar_const(x) or (isinstance(x, tir.PrimExpr) and x.dtype.is_float_scalar)


def is_floating_scalar(x):
    return _is_floating_scalar_const(x) or (
        isinstance(x, tir.PrimExpr) and x.dtype.is_floating_scalar
    )
...
def is_scalar(x):
    if PySimInfo.current is None:
        ...
        return is_integer_scalar(x) or is_floating_scalar(x)
...
def is_vector_or_mask(x):
    return isinstance(x, (PyVar, tir.PrimExpr)) and x.dtype.is_vector


def is_vector(x):
    return is_vector_or_mask(x) and not x.dtype.is_bool
...
def within_range(value, dtype=None, low=None, high=None):
    ...
    if dtype is not None:
        ...
        if get_dtype(value).is_floating and not dtype.is_floating:
            return False
...
def _get_most_fit_vdtype(x, candidate_vdtypes):
    ...
    candidate_vdtypes_str = f"({', '.join(str(x) for x in candidate_vdtypes)})"
    raise ValueError(f'Can\'t broadcast "{value_or_dtype}" to any of "{candidate_vdtypes_str}".')
