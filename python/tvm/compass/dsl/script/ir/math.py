...
from .utils import assert_not_bfloat16_scalar
...
def exp(x):
    ...
    assert dtype.is_floating, "Only support floating input."
...
def _py_ceildiv(x, y):
    ...
    return PyVar(np.trunc((x + y - one) / y), DataType(x.dtype))
...
def pow(x, exponent):  # pylint: disable=redefined-builtin
    ...
    dtype = x.dtype
    assert_not_bfloat16_scalar(dtype)
    assert dtype.is_floating, "Only support floating instruction."
...
def erf(x):
    ...
    .. code-block:: python
        ...
        vc = S.erf(va)
        scalar_b = S.erf(1.23456)

    See Also
    --------
    - Zhouyi Compass OpenCL Programming Guide: erf
    """
    dtype = get_dtype(x)
    assert_not_bfloat16_scalar(dtype)
    assert dtype.is_floating, "Only support floating input."
    return tir.call_extern(dtype, "erf" if dtype.is_scalar else "verf", x)


@register_ir_api
def _py_erf(x):
    return PyVar(special.erf(x), get_dtype(x))
...
def exp2(x):
    ...
    .. code-block:: python
        ...
        vc = S.exp2(va)
        scalar_b = S.exp2(1.23456)

    See Also
    --------
    - Zhouyi Compass OpenCL Programming Guide: exp2
    """
    dtype = get_dtype(x)
    assert_not_bfloat16_scalar(dtype)
    assert dtype.is_floating, "Only support floating input."
    return tir.call_extern(dtype, "exp2" if dtype.is_scalar else "vexp2", x)


@register_ir_api
def _py_exp2(x):
    return PyVar(np.exp2(x), get_dtype(x))
