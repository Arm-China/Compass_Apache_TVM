...
def vconcat(inps, part="all"):
    ...
    Supported DType
    ---------------
        "int8/16/32", "uint8/16/32", "float16/32", "bfloat16", "bool8/16/32".
    ...
    .. code-block:: python
        ...
        maskc = S.vconcat((mask0, mask1))
        maskc = S.vconcat((mask0, mask1), "low")
...
def vbcast(x, mask=None, lanes=None):
    ...
    Supported DType
    ---------------
        "int8/16/32", "uint8/16/32", "float16/32", "bfloat16", "bool".
...
def _py_vtbl(table, indices):
    ...
    return PyVar(r, DataType(table[0].dtype))
