...
"""Logger of Zhouyi Compass extension."""
import os
import sys
import logging
import functools
...
class _Logger:
    """Default level: INFO, default handler: stream."""
    ...
    def __init__(self):
        ...
        default_level = os.getenv("CPS_TVM_LOG_LEVEL", "INFO")
    ...
    def set_log_file(self, file_name):
        ...
        self.logger.info("Compass log dump to %s", file_path)
        for h in self.logger.handlers:
...
def set_logger(log_file=None, log_level=None):
    ...
    if log_file is None:
        log_file = os.getenv("CPS_TVM_LOG_FILE")
    if log_level is None:
        log_level = os.getenv("CPS_TVM_LOG_LEVEL")
    if log_file:
