...
def arg(name: str, obj: Union[Var, Buffer]) -> Union[Var, Buffer]:
    ...
    if isinstance(obj, tir.Pointer):
        pointer = tir.Pointer(obj.dtype, obj.scope, name=name)
        _ffi_api.PtrArg(name, pointer.base if pointer.buffer is None else pointer.buffer)
        return pointer
    ...
    *,
    ...
    if step is not None:
        if isinstance(step, Integral):
            step = IntImm(str(getattr(start, "dtype", "int32")), step)
    ...
    for index in indices:
        if isinstance(index, slice):
            ...
            lanes = Analyzer().simplify(  # pylint: disable=redefined-outer-name
                (index.stop - start + step - 1) // step
            )
            start = (offset + start) if offset is not None else start
            if lanes == 1:
                expr_indices.append(start)
            else:
                expr_indices.append(ramp(start, step, lanes))
        else:
            expr_indices.append((offset + index) if offset is not None else index)
