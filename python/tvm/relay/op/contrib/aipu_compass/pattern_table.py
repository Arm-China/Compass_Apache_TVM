...
from tvm.relay.dataflow_pattern import is_constant, is_op, wildcard, is_var, is_tuple_get_item
from tvm.relay.dataflow_pattern import is_tuple
from tvm.relay.backend.contrib.aipu_compass import AipuCompassConfig
from .utils import get_activation_str, unpack_commutative_args, is_scalar_and_close, peel_hardswish
...
def _gelu_pattern():
    ...
    def check(gelu: relay.Call):
        ...
        if not is_scalar_and_close(half, 0.5):
            return False
        if not is_scalar_and_close(one, 1):
            return False
        if not is_scalar_and_close(sqrt2, math.sqrt(2)):
            return False
        yield  # The Compass OP Spec check code must be placed after this statement.
        return True
...
def _layernorm_pattern0():
    ...
    def check(layernorm: relay.Call):
        ...
        if mean_axis0 != mean_axis1:
            return False
        if not is_scalar_and_close(power_num, 0.5):
            return False
...
def _check_hardswish(hardswish: relay.Call):
    ...
    if not is_scalar_and_close(six_const, ref):
        return False
