...
from tvm.relay.backend.contrib.aipu_compass import (
    AipuCompass,
    AipuCompassConfig,
    aipu_builder,
    ExecutionEngine,
)
from tvm.relay.backend.contrib.aipu_compass.config import AipuCompassFunctionConfig
...
def is_partition_all_to_compass(ir_mod):
    ...
    def _analysis_no_call(t):
        if isinstance(t, relay.Call):
            assert relay.pretty_print(t.op).startswith(
                "@tvmgen_default_aipu_compass_main_"
            ), "Some operators aren't partitioned to Compass NPU subgraph."

    relay.analysis.post_order_visit(ir_mod["main"], _analysis_no_call)


def compare_relay_opt_float_result(
    mod, *inputs, compare=None, cfg=None, compare_threshold=0.99, golden=None
):
    """Compare relay output with opt float forward"""
    cfg_str = """
    [Common]
    ...
    if golden is None:
        golden = relay.create_executor("vm", mod=mod).evaluate()(*inputs)

    if compare is None:
        cos_distance = compute_cos_distance(outputs[0].numpy(), golden.numpy())
        assert (
            cos_distance >= compare_threshold
        ), f"Compare Failed, Real {cos_distance} vs Golden {compare_threshold}"
    else:
...
def run_op_case(work_dir, case_file_path, target):
    ...
    for i, gt in enumerate(gts):
        ...
        output = np.fromfile(os.path.join(work_dir, out_bin), dtype=gt.dtype).reshape(gt.shape)
        cos_dis = compute_cos_distance(output, gt)
        print(f"Cosine Distance: {cos_dis}")
        if cos_dis > 0.99:
            return
