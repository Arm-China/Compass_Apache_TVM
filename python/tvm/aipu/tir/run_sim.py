...
"""Implement the simple run through AIPU simulator."""
import os
import textwrap
from ..script.pysim import _set_run_sim
...
def _gen_prim_func_info(inputs, consts, outputs, descs):
    ...
    for i, (name, x) in enumerate(inputs):
        ret.param_infos[idx] = ParamInfo(name, str(x.dtype), "input_tensor", tensor_idx=i)
        idx += 1

    for name, x in consts:
        ret.param_infos[idx] = ParamInfo(name, str(x.dtype), "const_tensor")
        idx += 1

    for i, (name, x) in enumerate(outputs):
        ret.param_infos[idx] = ParamInfo(name, str(x.dtype), "output_tensor", tensor_idx=i)
        idx += 1

    for name, _ in descs:
        ret.param_infos[idx] = ParamInfo(name, "uint32", "descriptor")
        idx += 1

    return ret


def run_sim(aipu_info, out_dir, code_snippet, inputs=None, outputs=None, consts=None, descs=None):
    """Simple run the specified code snippet through AIPU simulator."""
    ...
    compile_c(c_code, aipu_info.name, "", out_path, os.path.dirname(out_path))

    prim_func_info = _gen_prim_func_info(inputs, consts, outputs, descs)
    executor = Executor(prim_func_info, out_dir, aipu_info, False, only_sync_diff=True)
...
_set_run_sim(run_sim)  # To solve the circular import error.
