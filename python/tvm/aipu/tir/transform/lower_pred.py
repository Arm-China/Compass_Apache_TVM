...
"""Lower the standard TIR predicate nodes to the corresponding representation of AIPU."""
...
class _Mutator(tir.StmtExprMutator):
    # AIPU's predicate value is 32-bit, for operand whose element is "word"(e.g., int32x8) the last
    # bit of every 4 bits valid(e.g., 0bxxx1: enable, 0bxxx0: disable), for "half" operand (e.g.,
    # int16x16) the last bit of every 2 bits valid(e.g., 0bx1: enable, 0bx0: disable), for "byte"
    # operand(e.g., int8x32) every bit valid(e.g., 0b1: enable, 0b0: disable).

    def _mutate_const_pred(self, call):
        # Convert the boolean array to the predicate representation of AIPU.
        lanes = DataType(call.dtype).lanes
        ret = 0
    ...
    def _mutate_low_true_pred(self, call):
        n, lanes = call.args[0], DataType(call.dtype).lanes
