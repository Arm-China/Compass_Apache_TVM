...
from tvm import tir, ir, DataType, get_range
from ... import script as S


def _vcvt(x):
    assert x.dtype in ("float32x8", "int32x8")
    ret_vdtype = "float32x8" if x.dtype == "int32x8" else "int32x8"
    return tir.call_extern(ret_vdtype, "__vcvt", x)
...
def _reinterpret_or_nsr(x, to_dtype, saturate):
    ...
    shift = S.cast(0, DataType(x.dtype).with_uint())
    to_dtype = DataType(to_dtype)
...
class _Mutator(tir.StmtExprMutator):
    ...
    def _cast_to_narrower_with_merge(self, inputs, to_dtype, saturate):
        ...
        if from_dtype in ("int32x8", "uint32x8"):
            ...
            return _vcvtd(inputs[0], inputs[1])
        ...
        # 4.2 To 16-bit float, e.g., (fp32x8, fp32x8) -> fp16x16.
        assert to_dtype == "float16x16" and input_cnt == 2, msg
        return _vcvtd(inputs[0], inputs[1])
    ...
    def _cast_from_fp16(self, x, to_dtype, part, saturate, msg):
        ...
        if to_dtype == "int32x8":
            vcvtue = lambda expr: tir.call_extern("int32x8", "__vcvtue_tw", expr)
            vcvtuo = lambda expr: tir.call_extern("int32x8", "__vcvtuo_tw", expr)

        vcvtul = lambda expr: S.vzip(vcvtue(expr), vcvtuo(expr), part="low")
        vcvtuh = lambda expr: S.vzip(vcvtue(expr), vcvtuo(expr), part="high")
        if self._aipu_info.is_v3:
            ...
            if to_dtype == "int32x8":
                vcvtul = lambda expr: tir.call_extern("int32x8", "__vcvtul_tw", expr)
                vcvtuh = lambda expr: tir.call_extern("int32x8", "__vcvtuh_tw", expr)

        # 1. To 32-bit sign integer or float, e.g., fp16x16 -> i32x8, fp16x16 -> fp32x8.
        ...
        # 2. To 32-bit unsigned integer, e.g., fp16x16 -> u32x8.
    ...
    def _cast_from_fp32(self, x, to_dtype, part, saturate, msg):
        ...
        # 3. To 16-bit float, e.g., fp32x8 -> fp16x16.
        assert to_dtype == "float16x16" and part == "all", msg
        useless = x
        return _vcvtd(x, useless)
    ...
    def _mutate_vcast(self, call):
        ...
        if len(inputs) > 1:
            ...
            return self._cast_to_narrower_with_merge(inputs, to_dtype_str, saturate)

        # From here, the input of cast must be a single vector expression.
        x = inputs[0]
        from_dtype_str = x.dtype
        ...
        if from_dtype.bits == 8 and from_dtype.is_integer:
            return self._cast_from_i8_or_u8(x, from_dtype_str, to_dtype_str, part, saturate, msg)

        # 4. From 16-bit integer, e.g., i16x16 -> i8x32, u16x16 -> fp32x8.
        if from_dtype.bits == 16 and from_dtype.is_integer:
            return self._cast_from_i16_or_u16(x, from_dtype_str, to_dtype_str, part, saturate, msg)

        # 5. From 32-bit integer, e.g., i32x8 -> i8x32, u32x8 -> i16x16, i32x8 -> fp32x8.
        if from_dtype.bits == 32 and from_dtype.is_integer:
            return self._cast_from_i32_or_u32(x, from_dtype_str, to_dtype_str, part, saturate, msg)

        # 6. From 16-bit float, e.g., fp16x16 -> i32x8, fp16x16 -> fp32x8.
        if from_dtype.is_float16:
            return self._cast_from_fp16(x, to_dtype_str, part, saturate, msg)

        # 7. From 32-bit float, e.g., fp32x8 -> i32x8, fp32x8 -> u8x32, fp32x8 -> fp16x16.
        assert from_dtype.is_float32, msg
        return self._cast_from_fp32(x, to_dtype_str, part, saturate, msg)
...
class LowerVectorCast:
    ...
    def transform_function(self, func, ir_mod, pass_ctx):  # pylint: disable=unused-argument
        ...
        return func.with_body(_Mutator(self._aipu_info).visit(func.body), span=func.span)
