...
class _Mutator(tir.StmtExprMutator):
    def _mutate_vabs(self, call):
        ...
        ret_vdtype = DataType(call.dtype)
    ...
    def _mutate_vadd_vsub(self, call):
        ...
        new_args = list(call.args)
        for i in range(2, 4):
            if DataType(new_args[i].dtype) != ret_vdtype:
    ...
    def _mutate_vsl(self, call):
        shift = call.args[3]
        shift_vdtype = DataType(shift.dtype)
        ...
        x_vdtype = DataType(call.args[2].dtype)
    ...
    def _mutate_vnsr(self, call):
        ...
        with_saturate = bool(call.args[4])
        x_vdtype = DataType(call.args[1].dtype)
        ret_vdtype = DataType(call.dtype)
    ...
    def _mutate_vload_gather(self, call):
        ...
        if any(not DataType(x.dtype).is_uint16 for x in offsets):
            new_args = list(call.args)
