...
def __getattr__(name):
    ...
    from tvm.aipu.script.ir.base import register_ir_api as ria

    if type_code == "bool":
        from tvm.aipu.script.ir.conversion import _gen_bool_builder_impl, _gen_empty_python_impl
    ...
    from tvm.aipu.script.ir.conversion import _gen_dtype_builder_impl, _gen_dtype_python_impl

    globals()[_alias] = globals()[_dtype] = ria(_gen_dtype_builder_impl(_dtype))
    ria(_gen_dtype_python_impl(_dtype))

    from tvm.aipu.script.ir.conversion import (
        _gen_size_dtype_builder_impl,
        _gen_size_dtype_python_impl,
    )
