...
def _check_unsupported_direct_cast(from_dtype, to_dtype):
    if (from_dtype.is_float16 and to_dtype.is_integer and to_dtype.bits < 32) or (
        from_dtype.is_integer and from_dtype.bits < 32 and to_dtype.is_float16
    ):
        raise ValueError(
            f'Unsupported cast directly from "{from_dtype}" to "{to_dtype}", because there isn\'t '
            "any hardware conversion instructions between float16x16 and narrower bits integer, so "
            'you need to implement it manually through 32-bit and multiple invocations of "cast".'
        )
...
def cast(x, dtype, part="all", saturate=None):
    ...
    saturate : Optional[bool]
        ...
        ``float16``/``float32`` to ``int32`` stage, ``False`` for ``int32`` to the target data type
        stage, ``False`` when casting in other situations.
...
def _round_saturate_if_needed(x, to_dtype, saturate):
    if to_dtype.is_float:
        return x
    ...
    if from_dtype.is_float:
        ret = np.round(x) if from_dtype.is_vector else x
...
def _py_cast(x, dtype, part="all", saturate=None):
    ...
    # 1. Scalar and broadcast scenarios.
    if is_scalar(x):
        ret = _round_saturate_if_needed(x, to_dtype, saturate)
    ...
    if isinstance(x, (list, tuple)):
        ...
        return PyVar(_round_saturate_if_needed(ret, to_dtype, saturate), ret_vdtype)
...
def reinterpret(x, dtype):
    ...
    if not isinstance(x, tir.Pointer):
        ...
        assert DataType(x.dtype).total_bits == dtype.total_bits, msg
