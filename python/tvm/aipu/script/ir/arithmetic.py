...
def vadd(x, y, mask=None, saturate=False, out_sign=None, r=None):
    ...
    assert not (x_vdtype.is_float and saturate), "Saturate is meaningless for floating."
...
@register_ir_api
def vabs(x, mask=None, saturate=False):
...
@register_ir_api
def _py_vabs(x, mask=None, saturate=False):
    if saturate:
...
def _py_vdot(x, y, mask=None):
    ...
    for i in range(x_lanes):
        if mask[i]:
            ret[i // 2] += x[i] * y[i]
    ret = np.clip(ret, *get_range(ret_vdtype.element_of))
...
def vqdot(x, y, mask=None):
    ...
    ret_vdtype = double_elem_width(x_vdtype)
    ret_vdtype = double_elem_width(ret_vdtype) if x_vdtype.is_integer else ret_vdtype
    ret_vdtype = ret_vdtype.with_int() if x_vdtype.is_int or y_vdtype.is_int else ret_vdtype
    mask = canonicalize_mask(mask, x_vdtype.lanes)
...
def _py_vqdot(x, y, mask=None):
    ...
    x_lanes = x.dtype.lanes
    ret_vdtype = double_elem_width(x.dtype)
    ret_vdtype = double_elem_width(ret_vdtype) if x.dtype.is_integer else ret_vdtype
    ret_vdtype = ret_vdtype.with_int() if x.dtype.is_int or y.dtype.is_int else ret_vdtype
...
def vqdpa(acc, x, y, mask=None):
    ...
    assert ((acc_vdtype.is_integer and x_vdtype.is_integer) and is_valid_lanes_bits(4)) or (
        (acc_vdtype.is_float and x_vdtype.is_float) and is_valid_lanes_bits(2)
    ), f'Argument type mismatch: 0-th: "{acc_vdtype}" vs. 1-th: "{x_vdtype}".'
...
@register_ir_api
def vmml(ptr, x, y):
    """Performs a mixture precision 4x4 matrix multiply and addition for float16x16 vector
    x (row-major) and y (column-major). The result pointer ``ptr`` is the address of float32x16 with
    row-major. This behavior is the same as ``ptr[:?] = matrix_multiply(x, y)``.
    ...
    msg = f'The data type of the 2nd arg expect "float16x16", but got: "{x.dtype}".'
    assert x.dtype == "float16x16", msg
    msg = f'The data type of the 3rd arg expect "float16x16", but got: "{y.dtype}".'
    assert y.dtype == "float16x16", msg
...
def vmma(acc_ptr, x, y):
    """Performs a mixture precision 4x4 matrix multiply and addition for float16x16 vector
    x (row-major) and y (column-major). The result pointer ``acc_ptr`` is the address of float32x16
    with row-major. This behavior is the same as ``acc_ptr[:?] += matrix_multiply(x, y)``.
...
def fma(acc, x, y, mask=None):
    ...
    if all(is_scalar(arg) for arg in (acc, x, y)):
        ...
        acc_dtype = DataType(acc.dtype)
        msg = "Only support float32 instruction."
        assert acc_dtype.is_float32 and DataType(y.dtype).is_float32, msg
...
def _py_fma(acc, x, y, mask=None):
    if is_scalar(acc):
        ...
        return PyVar(acc_fp64 + x * y, dtype="float32")
...
def vfmae(acc, x, y, mask=None):
    ...
    acc_vdtype, x_vdtype = DataType(acc.dtype), DataType(x.dtype)
    assert_neither_flexible_nor_multiple_width_vector(x_vdtype)
    assert (acc_vdtype.is_float32 and x_vdtype.is_float16) and (
...
def clip(x, min_val, max_val, mask=None):
    ...
    # For float, using "vsel" instead of "vmax" and "vmin", because the AIPU behavior of "vmax"
    # or "vmin" is unexpected when one operand is NaN, and another is a normal value, its result
    # is the normal value. In other words, "vmax" or "vmin" doesn't reserve NaN.
