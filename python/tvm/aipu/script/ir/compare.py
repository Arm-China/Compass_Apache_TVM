...
import builtins
import numpy as np
from tvm import tir, target as tgt, DataType
...
def vceq(x, y, mask=None):
    ...
    mask = canonicalize_mask(mask, DataType(x.dtype).lanes)
...
def _py_vceq(x, y, mask=None):
    ...
    return PyVar(x == y, f"boolx{x.dtype.lanes}", mask, False)
...
def isnan(x, mask=None):
    ...
    assert x_dtype.is_float, f'The data type of the 1st arg expect floating, but got: "{x.dtype}".'
...
def _py_isnan(x, mask=None):
    ...
    mask = canonicalize_mask(mask, x.dtype.lanes)
    return PyVar(np.isnan(x), f"boolx{x.dtype.lanes}", mask, False)
...
def _py_isinf(x, mask=None):
    ...
    mask = canonicalize_mask(mask, x.dtype.lanes)
    return PyVar(np.isinf(x), f"boolx{x.dtype.lanes}", mask, False)
...
def _py_max(x, y, mask=None, r=None):
    if is_scalar(x) and is_scalar(y):
        x, y = binary_op_match_types(x, y)
        return PyVar(np.max([x, y]), x.dtype)
...
def _py_min(x, y, mask=None, r=None):
    if is_scalar(x) and is_scalar(y):
        x, y = binary_op_match_types(x, y)
        return PyVar(np.min([x, y]), x.dtype)
...
def vrpmax(x, mask=None, return_idx=False):
    ...
    """
    if return_idx:
        aipu_info = tgt.AipuInfo.current()
        msg = f'The arg "return_idx" only support V3 architecture, but got: "{aipu_info.name}".'
        assert aipu_info.is_v3, msg
