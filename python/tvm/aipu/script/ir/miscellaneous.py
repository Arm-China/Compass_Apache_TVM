...
def _py_const_mask(x):
    return PyVar(canonicalize_mask(x, len(x)), f"boolx{len(x)}")
...
def _py_tail_mask(n, lanes):
    ...
    return PyVar([True] * n + [False] * (lanes - n), f"boolx{lanes}")
...
def _py_get_local_size():
    return PyVar(PySimInfo.current.local_size, "int32")
...
def _py_get_local_id():
    return PyVar(PySimInfo.current.thread_local_data.id, "int32")
...
@register_ir_api
def aiff(ctrl_desc, param_desc, act_desc, mode=0):
    ...
    assert isinstance(mode, int), 'The arg "mode" expect an integer scalar constant.'
    valid_modes = (0, 4) if tgt.AipuInfo.current().is_v3 else (0,)
    assert mode in valid_modes, f'The arg "mode" expect one of {valid_modes}, but got: "{mode}".'

    return tir.call_extern("void", "AIFF", ctrl_desc, param_desc, act_desc, mode)


@register_ir_api
def _py_aiff(ctrl_desc, param_desc, act_desc, mode=0):
    ...
    pysim_run_sim(f"AIFF(ctrl, param, act, {mode});", descs=descs)


@register_ir_api
def async_aiff(ctrl_desc, param_desc, act_desc, event, mode=0):
    ...
    return tir.call_extern("void", "ASYNC_AIFF", ctrl_desc, param_desc, act_desc, event, mode)


@register_ir_api
def _py_async_aiff(ctrl_desc, param_desc, act_desc, event, mode=0):
    ctrl = ctrl_desc.get_current_desc_chain()
    param = param_desc.get_current_desc_chain()
    act = act_desc.get_current_desc_chain()
    descs = [("ctrl", ctrl), ("param", param), ("act", act)]
...
def _check_arg_type(fmt, args):
    ...
    def _callback(invoke_pos, spcfr):
        ...
        if spcfr == "%c":
            ...
            assert isinstance(arg, tir.PrimExpr) and DataType(arg.dtype).total_bytes == 32, msg
            ...
            dtype = DataType(arg.dtype)
            assert dtype.is_float and dtype.total_bytes == 32, msg
        cur_arg_idx += 1
...
def _format_arg(spcfr, arg):
    ...
    # AIPU always keep 6 digits following the decimal point, and it won't do rounding, e.g.
