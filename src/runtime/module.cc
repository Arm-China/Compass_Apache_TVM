...
Module Module::LoadFromFile(const String& file_name, const String& format) {
  std::string file_path = AbsPath(IsDir(file_name) ? file_name + "/nn_model.so" : file_name);
  std::string fmt = GetFileFormat(file_path, format);
  ICHECK(fmt.length() != 0) << "Cannot deduce format of file " << file_path;
  if (fmt == "dll" || fmt == "dylib" || fmt == "dso") {
    fmt = "so";
  }
  std::string load_f_name = "runtime.module.loadfile_" + fmt;
  VLOG(1) << "Loading module from '" << file_path << "' of format '" << fmt << "'";
  ...
  Module m = (*f)(file_path, format).cast<Module>();
  std::string dir = Dirname(file_path);
  for (auto cps_mod : GetAllCompassModule(m)) {
    static_cast<CompassModule*>(cps_mod.operator->())->ReviseCpsBinPath(dir);
  }
  return m;
}
