...
Map<IterVar, Range> InferBound(const Schedule& sch) {
  ...
  for (size_t i = sch->stages.size(); i != 0; --i) {
    ...
    if (stage.IsComputeInside() == false) {
      ...
      const Array<IterVar>& attach_path = ctx.attach_path.at(stage->op);
      // The iteration variables which aren't leaf are still needed to be
      // processed, because their bounds maybe needed later, e.g., the function
      // "PassDownDomain".
      for (auto iv : stage->all_iter_vars) {
        // The iteration variables generated by other primitives following the
        // primitive "compute_inside" won't record in the bound map, so here
        // just ignore them, and infer their bounds through "PassDownDomain".
        if (stage->attach_bound_map.count(iv) == 0) continue;

        const IterVar& ref_iv = stage->attach_bound_map[iv];
        if (attach_path.Contains(ref_iv)) {
          ret[iv] = Range::FromMinExtent(ref_iv->var, 1);
        } else {
          ret[iv] = ret[ref_iv];
        }
        analyzer.Bind(iv->var, ret[iv]);
      }
    }
