...
TVM_FFI_REGISTER_GLOBAL("tir.SizeVar")
    .set_body_typed([](String name_hint, ffi::AnyView type, Span span) {
      if (type.as<Type>()) {
        return SizeVar(name_hint, type.cast<Type>(), span);
      } else {
        return SizeVar(name_hint, type.cast<DataType>(), span);
      }
    });
...
TVM_FFI_REGISTER_GLOBAL("tir.ConvertToPrimExpr").set_body_typed([](PrimExpr expr) { return expr; });
