...
static inline PrimExpr ShallowCopy(PrimExpr predicate) {
  if (auto* call = predicate.as<CallNode>()) {
    return Call(call->dtype, call->op, call->args, call->span);
  }
  return Bool(Downcast<Bool>(predicate)->value);
}
...
class Vectorizer : public StmtMutator, public ExprFunctor<PrimExpr(const PrimExpr&)> {
 public:
  ...
  Vectorizer(Var var, PrimExpr var_lanes, PrimExpr predicate)
