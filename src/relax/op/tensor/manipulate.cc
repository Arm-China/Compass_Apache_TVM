...
Expr reverse_sequence(Expr data, Expr seq_lengths, int seq_axis, int batch_axis) {
  ObjectPtr<ReverseSequenceAttrs> attrs = make_object<ReverseSequenceAttrs>();
...
TVM_FFI_REGISTER_GLOBAL("relax.op.reverse_sequence").set_body_typed(reverse_sequence);

StructInfo InferStructInfoReverseSequence(const Call& call, const BlockBuilder& ctx) {
  arith::Analyzer* analyzer = ctx->GetAnalyzer();
  Array<TensorStructInfo> input_sinfo = GetInputTensorStructInfo(call, ctx);
  TensorStructInfo data_sinfo = input_sinfo[0];
  TensorStructInfo seq_lengths_sinfo = input_sinfo[1];

  ICHECK_EQ(seq_lengths_sinfo->ndim, 1) << "For reverse_sequnece, seq_lengths must be a 1D vector";
  ICHECK(seq_lengths_sinfo->dtype.is_int())
      << "For reverse_sequnece, seq_lengths must be tensor of integer";

  const auto* data_shape = data_sinfo->shape.as<ShapeExprNode>();
  const auto* seq_lengths_shape = seq_lengths_sinfo->shape.as<ShapeExprNode>();
  const auto* attrs = call->attrs.as<ReverseSequenceAttrs>();
  const int ndim = data_sinfo->ndim;

  int batch_axis = attrs->batch_axis;
  ...
  if (batch_axis < 0) {
    batch_axis = ndim + batch_axis;
  }

  const int seq_axis = attrs->seq_axis;
  ...
  if (analyzer->CanProve(seq_lengths_shape->values[0] != data_shape->values[batch_axis])) {
    ctx->ReportFatal(
        Diagnostic::Error(call)
        << "For reverse_sequnece seq_lengths size should match with dimension of batch axis"
        << ", but got dimension of batch_axis = " << data_shape->values[batch_axis]
        << ", and seq_length size = " << seq_lengths_shape->values[0]);
  }
  return TensorStructInfo(ShapeExpr(data_shape->values), data_sinfo->dtype, data_sinfo->vdevice);
}

InferLayoutOutput InferLayoutReverseSequence(const Call& call,
                                             const Map<String, Array<String>>& desired_layouts,
                                             const VarLayoutMap& var_layout_map) {
  ICHECK(NoDesiredLayout(call, desired_layouts));
  const auto* attrs = call->attrs.as<ReverseSequenceAttrs>();
  ...
  ObjectPtr<ReverseSequenceAttrs> new_attrs = make_object<ReverseSequenceAttrs>(*attrs);
  new_attrs->seq_axis = FindAxis(layout->layout, attrs->seq_axis);
  new_attrs->batch_axis = FindAxis(layout->layout, attrs->batch_axis);
...
TVM_REGISTER_OP("relax.reverse_sequence")
    ...
    .set_attr<FInferStructInfo>("FInferStructInfo", InferStructInfoReverseSequence)
    .set_attr<FRelaxInferLayout>("FRelaxInferLayout", InferLayoutReverseSequence)
    .set_attr<Bool>("FPurity", Bool(true));
